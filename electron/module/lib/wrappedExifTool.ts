...
const writeDateTimeWithTimezone = async ({
  filePath,
  description,
  dateTimeOriginal,
  timezoneOffset,
  ...
  const exifTool = await getExiftoolInstance(); // シングルトンインスタンスを取得
  // EXIF情報を書き込む
...
export const setExifToBuffer = async (
  ...
  const write_r = fs.writeFileSyncSafe(tmpFile.path, buffer);
  if (write_r.isErr()) {
  ...
  // 一時ファイルを削除
  const delete_r = await fs.unlinkSafe(tmpFile.path);
  if (delete_r.isErr()) {
    return neverthrow.err(
      new Error('Failed to delete temporary file', { cause: delete_r.error }),
    );
  }
  return neverthrow.ok(read_r.value);
};
...
export const readExifByBuffer = async (
  buffer: Buffer,
): Promise<neverthrow.Result<exiftool.Tags, Error>> => {
  // 一時ファイルに書き込み
  const tmpFile = await tmp.file();
  const write_r = await fs.writeFileSyncSafe(tmpFile.path, buffer);
  if (write_r.isErr()) {
    return neverthrow.err(
      new Error('Failed to write buffer to temporary file', {
        cause: write_r.error,
  ...
  // 一時ファイルを削除
  const unlink_r = await fs.unlinkSafe(tmpFile.path);
  if (unlink_r.isErr()) {
    return neverthrow.err(
